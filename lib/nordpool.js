...
async function fetchHourlyPrices(node, mock = false) {
    ...
    console.log("subtract CC: " + node.subtractFromCountryCode);
    console.log("subtract fees: " + node.subtractFees);

    if (node.subtractFromCountryCode) {
        console.log("Subtract from enabled, fees=" + node.subtractFees);
        find_url = url_template.replace("{CC}", node.subtractFromCountryCode)
        obj = Object.values(queries_response.data.tree).filter(v => v.url.toLowerCase() == find_url.toLowerCase())[0]
        if (!obj) {
            node.error("Country Code '" + node.countryCode + "' not found from queries API, looked for URL '" + find_url + "'");
            return null;
        ...
        console.log("found page id: " + obj.id);
        response = await axios.get(page_url.replace("{ID}", obj.id));
        let subtractFromPrices = response.data.data.Rows.slice(0,24).map(row => parseFloat(row.Columns[0].Value.replace(',','.')));
        let newPrices = [];
        console.log("Old prices: " + JSON.stringify(result.prices, null, 4));
        for (const [key, value] of Object.entries(result.prices)) {
            console.log(key, value);
            let newValue = value - subtractFromPrices[key] + node.subtractFees;
